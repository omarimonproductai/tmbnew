...
- [x] **1.0 Backend proxies (TMB Planner + Photon geocoder)**
...
- [x] **2.0 New "Ruta" mode + icon redesign**
  - [x] 2.1 Update `src/hooks/useDisplayMode.ts`: extend the union type to
  ...
  - [x] 2.2 Update `src/components/ModeToggle.tsx`: redesign with 4 buttons.
        Each button renders an inline SVG (Ruta = nav arrow, Línies =
        bus+rectangle, Aprop meu = radar concèntric, Favorits = star).
  - [x] 2.3 Add responsive label: SVG icon always; text label visible only
        at `min-width: 641px` via CSS class (e.g. `.mode-btn__label`
        with `display: none` until tablet breakpoint).
  - [x] 2.4 Update `src/components/ModeToggle.test.tsx`: test that 4
        buttons render, that clicking each calls onChange with the right
        value, and that `'route'` is recognised.
  - [x] 2.5 Create `src/components/RoutePlannerView.tsx` shell (placeholder
        text "Route planner – coming soon" inside `<main className="app-main">`).
  - [x] 2.6 Update `src/App.tsx` to render `RoutePlannerView` when
        `mode === 'route'`.
  - [x] 2.7 Update `src/App.css` for the new ModeToggle layout (icon column
        + optional label, sizing tweaks for 4 items).
  - [x] 2.8 Smoke-check: switch between the 4 modes in dev, confirm
        navigation works and each view still renders correctly.

- [x] **3.0 Search form (A → B)**
  - [x] 3.1 Create `src/services/geocode.ts` with `searchPlaces(query)` that
        calls `/api/geocode/search?q=…`. Returns typed `GeocodeResult[]`.
  - [x] 3.2 Create `src/types/geocode.ts` with `GeocodeResult`
        `{ id, name, sub, lat, lng, osmType, osmKey }`.
  - [x] 3.3 Create `src/hooks/usePhotonSearch.ts` with 300ms debounce,
        min-3-char threshold, and abort on rapid retyping.
  - [x] 3.4 Create `src/hooks/usePlannerHistory.ts` with
        `add(destination)`, `list()` (max 5, MRU order), persistence on
        `tmb-planner-history-v1`.
  - [x] 3.5 Create `src/hooks/usePlannerModes.ts` with `{ metro, bus }`
        defaulting both to true, persisted on `tmb-planner-modes-v1`.
  - [x] 3.6 Create `src/components/RouteFieldRow.tsx`: blue dot (origin) +
        dashed line + red dot (destination), with editable inputs and a
        prop for the focused field.
  - [x] 3.7 Create `src/components/GeocodeDropdown.tsx`: renders Photon
        results with icon (POI/transit/street) + name (bold) + sub.
  - [x] 3.8 Create `src/components/PlannerModeFilters.tsx`: Metro/Bus pills.
        Prevent both being toggled off (show a small inline message).
  - [x] 3.9 Create `src/components/RouteSearchForm.tsx` that composes the
        field row, dropdowns (history when empty, geocode when typing),
        filter pills, and the swap button (↕).
  - [x] 3.10 GPS auto-fill: on mount, if geolocation grants, prefill
        `from` with `{ name: 'La meva ubicació', lat, lng }`. If denied,
        leave the field empty with placeholder and block the "Buscar"
        button.
  - [x] 3.11 Make origin editable: tapping the field clears the "La meva
        ubicació" sentinel and lets the user search for any address.
  - [x] 3.12 Swap button (↕) swaps the two values atomically.
  - [x] 3.13 Tests in `RouteSearchForm.test.tsx`: input debouncing, GPS
        auto-fill flow, swap behaviour, history shown when empty,
        Metro+Bus prevention.

- [x] **4.0 Result rendering on map**
  - [x] 4.1 Create `src/types/planner.ts` mirroring the normalised backend
        shape (`Itinerary`, `Leg`, `LegMode = 'WALK' | 'BUS' | 'METRO' | 'SUBWAY' | 'TRAM'`).
  - [x] 4.2 Create `src/services/planner.ts` with
        `getRoutePlan(from, to, modes): Promise<RoutePlan>`.
  - [x] 4.3 Create `src/hooks/useRoutePlan.ts` with explicit `trigger()`
        (no auto-fetch — only on Buscar). Surface loading / error / data.
  - [x] 4.4 Create `src/utils/polyline.ts` Google polyline decoder
        (5-precision). Add tests with at least 2 fixtures.
  - [x] 4.5 Create `src/components/RoutePlanTabs.tsx` with labelling
  ...
  - [x] 4.6 Create `src/components/RoutePlanMap.tsx`: render polyline per
  ...
  - [x] 4.7 Create `src/components/RouteLegPopup.tsx` with leg-specific
        content (e.g. "Pugeu al [V15] cap a [Av. Tibidabo]", "Baixa",
        "Transbord"). Bind to the relevant markers.
  - [x] 4.8 Create `src/components/RouteSummaryChip.tsx`: floats over the
        top of the map showing big duration + meta (arribada, transbords,
        m a peu).
  - [x] 4.9 Create `src/components/RouteRecenterButton.tsx` (bottom-right
        of the map) that re-applies fit-bounds to the route.
  - [x] 4.10 Create `src/components/PlannerEmptyState.tsx` for the
        loading spinner, no-route, fora-de-cobertura, and API error
        states; use it where appropriate inside `RoutePlannerView`.
  - [x] 4.11 When a result is loaded, the `RouteSearchForm` collapses to a
        compact 2-line summary at the top (origin + destination text,
        swap button optional). Tapping either line re-expands the field.
  - [x] 4.12 Tests in `RoutePlanTabs.test.tsx` covering labelling logic
        and dedupe; tests in `useRoutePlan.test.ts` mocking fetch.

- [x] **5.0 Last-mile Cooltra + contextual entry + polish**
  - [x] 5.1 Inside `RoutePlannerView`, call `useCooltraVehicles(true)` to
        always have the fleet available.
  - [x] 5.2 When a `RoutePlan` is loaded, compute the destination
        coordinates and filter the fleet by `haversine(dest, vehicle) ≤ 200`.
  - [x] 5.3 Pass the filtered list to `RoutePlanMap` and render via the
        existing `CooltraLayer` component.
  - [x] 5.4 Update `src/components/AproperMeuStopPopup.tsx` to add a
        button **"Ruta fins aquí"** that calls a callback prop with
        `{ name, lat, lng }`.
  - [x] 5.5 Update `src/components/StopPopup.tsx` likewise for the Línies
        view's stops.
  - [x] 5.6 Wire the callback in the parent views (`AproperMeuView`,
  ...
  - [x] 5.7 Handle the A → A edge case: if origin and destination are
        within 50m, show a friendly "Ja estàs aquí" message instead of
        calling the API.
  - [x] 5.8 Handle the no-GPS edge case: if `useGeolocation()` returns
  ...
  - [x] 5.10 Run `npm run lint && npm run build && npm test` clean before
        opening the integration PR.
  - [x] 5.11 Walk through `Open Questions` in the PRD once more; mark
        resolutions or move unresolved ones to a follow-up task list.
  - [x] 5.12 Open the integration PR from `feature/route-planner` to
